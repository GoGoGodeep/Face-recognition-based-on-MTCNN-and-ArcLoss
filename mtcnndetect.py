...
# P网络:
p_cls = 0.6     # 原为0.6
p_nms = 0.5     # 原为0.5 p网络在框人脸时，精度低，会画很多框，给高了会保留大量的框
# R网络：
r_cls = 0.6     # 原为0.6
r_nms = 0.5     # 原为0.5
# R网络：
...
class Detector:
    ...
    def __pnet_detect(self, image): # ★p网络全部是卷积，与输入图片大小无关，可输出任意形状图片
        ...
        while min_side_len > 12: # 直到缩放到小于等于12时停止
            ...
            offest = _offest[0].cpu().data  #[3, 203, 245] # 分组卷积特征图的通道、尺寸:C,W,H
    ...
    def __onet_detect(self, image, rnet_boxes):
        ...
        offset = _offset.cpu().data.numpy()     # (1, 14)
        ...
        for idx in idxs:    # 根据索引，遍历符合条件的框；1轴上的索引，恰为符合条件的置信度索引（0轴上索引此处用不到）
            ...
            lefteye_x = _x1 + ow * offset[idx][4]
            lefteye_y = _y1 + oh * offset[idx][5]
            righteye_x = _x1 + ow * offset[idx][6]
            righteye_y = _y1 + oh * offset[idx][7]
            nose_x = _x1 + ow * offset[idx][8]
            nose_y = _y1 + oh * offset[idx][9]
            leftmouth_x = _x1 + ow * offset[idx][10]
            leftmouth_y = _y1 + oh * offset[idx][11]
            rightmouth_x = _x1 + ow * offset[idx][12]
            rightmouth_y = _y1 + oh * offset[idx][13]
...
if __name__ == '__main__':
    ...
    for i in os.listdir(image_path):
        im = Image.open(image_path + i).convert('RGB')
        print("----------------------------")
